...
- **`cpu` / `cpu-omp`**: 行／ブロック単位で `BlockQ1_0` を float に戻し、入力ベクトルとの内積に使用する。
- **`cpu-blas`**: 上記と数学的に同等だが、`dot_q1_0_row` で **中間 float ブロックを経由しない**融合内積を行う（`Bonsai-8B-Q1_0` の律速に効く）。
...
- **8B を CPU で動かすため重い**場合がある。単スレッドは特に遅くなりがち（参考実装・検証向け）。**`cpu-omp`** 単体では Q1_0 行 GEMV がブロック dequant 主体のままとなり、実測ではスループットが低いことがある（本設計書の参考ベンチマーク参照）。実用的な試行は **`cpu-blas`**（OpenBLAS + 融合 Q1_0）を推奨。
...
実装の最終的な挙動は **`bonsai-8b/cpu/main.c`** のソースを正とする。**`cpu-omp`** / **`cpu-blas`** は派生であり、浮動小数の結合順などで数値差が出うる。スループット比較の際はビルドフラグ（`-march=native`、`-ffast-math`）と OpenBLAS スレッド設定に注意する。
