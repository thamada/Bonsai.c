...
**`bonsai-8b/gpu-cuda/` is an appendix outside this repo’s goals** (single C source, minimal dependencies). It splits across `main.c` + `kernels.cu` + `polarquant.c` + `gpu.h` and requires **CUDA Toolkit, an NVIDIA driver, and a physical GPU**. The reference implementation to read first is **`cpu/main.c`**.
...
| KV cache (`--polarquant`) | `kc_pack`, `vc_pack`, `kc_win`, `vc_win`, `kc_win_pos`, `vc_win_pos` | Full-series **pack** (K/V each **36 B/head** = indices 32 + **per-vector scale** 4, **Pi** rotation + Lloyd-Max **2-bit**, no QJL) + **F32 ring for last 64 tokens** (`FA_BR`, tag `win_pos[slot]==pos`). No full F32 `kc`/`vc` (startup log **72 B/head** = K+V pack total; pack+win ~**5×** VRAM savings, `attn=pack+win64`) |
...
6. **KV write** — With **`--polarquant`**: `kv_pq_compress_kernel` to pack (**`sc=max|y_rot|`** normalization) + `kv_write_ring_kernel` into the F32 ring. Otherwise D2D into F32 `kc`/`vc`.
7. **Attention** — `flash_attn_gqa_kernel`. With **`--polarquant`**: last 64 positions use F32 ring when tags match; older positions use pack (K: **`q·(Pi^T·sc·centroid)`**, V: restore via **`Pi^T`**) → `xb`.
...
| KV write | **`--polarquant`**: `kv_pq_compress_batch` + `kv_write_ring_batch` / default: `kv_write_batch` (F32) | pack + ring window, or full-series F32 |
| Attention | `flash_attn_prefill_gqa_kernel` | 1 block / `(token, head)`, causal mask `npos = t + 1` (with **`--polarquant`**, pack+win) |
...
The attention matrix `[n_heads, seq, seq]` is **never materialized**. K/V cache is scanned in **64-token (`FA_BR`) tiles** along the sequence; **online softmax** (running max `m` and sum `l`) updates the output. With **`--polarquant`**, each position uses the F32 ring when `win_pos[slot]==pos` and the position is within the last 64 tokens; otherwise pack reconstruction (**centroid×scale**).
...
- **`--polarquant`**: Prefill fills pack + ring; decode appends from `pos = n_prompt`. Prompts **≤ 64 tokens** stay mostly on the F32 ring; longer context relies on 2-bit pack approximation (the last 64 positions stay F32 for quality).
- **PolarQuant** ([arXiv:2502.02617](https://arxiv.org/abs/2502.02617)): **Pi** rotation + Lloyd-Max **2-bit** (no QJL). Auto-disabled if **`head_dim != 128`**. Legacy **TurboQuant** (QJL + pack+win) has been removed.
- **Pack-only** (without per-vector scale) can degrade quality on some prompts. Current builds use **per-vector scale** + **pack+win** to stay close to F32 output. **`--polarquant` decode throughput can be lower than default F32 KV** (table below is F32 KV).
...
CLI options (`-p`, `-n`, `-t`, `-k`, `-s`, `-l`) match the CPU builds. **`gpu-cuda` only:** **`--polarquant`** (PolarQuant pack KV + 64-token F32 ring, **pack+win** attention).
...
With **`--polarquant`**, KV VRAM drops by ~**5×** while decode may fall to roughly **~11–42 tok/s** (needs re-benchmarking). `-p "Hello, how are you?"` has been checked to match F32-default phrasing.

With the same prompt and `-t 0` (F32 KV), output matched **`cpu-blas`** (`Hello! I'm Bonsai, an AI assistant developed by PrismML.`). `-ffast-math` / `-use_fast_math` can change FP reduction order vs CPU builds; output still matched in this run.
...
7. `bonsai-8b/gpu-cuda/kernels.cu` — CUDA kernels (`gpu_forward_prefill` / `gpu_forward`, Flash Attention, PolarQuant compression, pack+win, etc.)  
8. `bonsai-8b/gpu-cuda/polarquant.c` — PolarQuant CPU reference (Lloyd-Max, **Pi** rotation, table setup)  
9. `bonsai-8b/gpu-cuda/gpu.h` — C API (`gpu_forward_prefill`, etc., `GpuConfig.polarquant_kv`)
