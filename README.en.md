...
| KV cache (`--turboquant`) | `kc_pack`, `vc_pack`, `kc_win`, `vc_win`, `kc_win_pos`, `vc_win_pos` | **Pack** (K: 48 B/head Polar+QJL, V: 32 B/head MSE only) + **F32 ring for last 64 tokens** (`FA_BR`, slot tag `win_pos[slot]==pos` avoids ring aliasing). No full-series F32 `kc`/`vc` (~**5–7×** VRAM savings). `TQ_F32_MIRROR=1` enables legacy full F32 mirror (debug) |
...
6. **KV write** — With **`--turboquant`**: compress to pack + copy into F32 ring window. Otherwise D2D into F32 `kc`/`vc`.
7. **Attention** — `flash_attn_gqa_kernel`. With **`--turboquant`**: last 64 positions use F32 ring when tags match; older positions use pack (K: `Pi^T·centroid` + QJL, V: `Pi^T·centroid`) → `xb`.
...
| Attention | `flash_attn_prefill_gqa_kernel` | 1 block / `(token, head)`, causal mask `npos = t + 1` (with **`--turboquant`**, pack + ring) |
...
The attention matrix `[n_heads, seq, seq]` is **never materialized**. K/V cache is scanned in **64-token (`FA_BR`) tiles** along the sequence; **online softmax** (running max `m` and sum `l`) updates the output. With **`--turboquant`**, each position uses the F32 ring when `win_pos[slot]==pos` and the position is within the last 64 tokens; otherwise pack reconstruction.
...
- **`--turboquant`**: Prefill fills pack + ring; decode appends from `pos = n_prompt`. Prompts **≤ 64 tokens** stay mostly on the F32 ring; longer context relies on pack approximation (quality ceiling: **`TQ_F32_MIRROR=1`**).
- **TurboQuant** ([arXiv:2504.19874](https://arxiv.org/abs/2504.19874)): K uses PolarQuant 2-bit + QJL; V uses PolarQuant only (MSE, no QJL per paper). **`--turboquant`** uses pack + a 64-token tagged F32 ring (~5–7× less KV VRAM than full F32). Auto-disabled if **`head_dim != 128`**. **`TQ_F32_MIRROR=1`** enables full-series F32 mirror (debug).
