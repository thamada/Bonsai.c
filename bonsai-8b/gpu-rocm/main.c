...
/*
 * Bonsai 系 dense デコーダ GGUF — AMD GPU ROCm / HIP 版（hipBLAS 不要）。
 *
 * 対象: Bonsai-8B-Q1_0.gguf（Q1_0 g128 + F32 norm 等）。テキスト処理のみ。
 * GPU 方針は gpu-cuda と同趣旨（Q1_0 + Q8_0 GEMV、Flash Attention）。
 * ビルド: gpu-rocm/Makefile（hipcc、GPU_ARCH は rocminfo 自動検出）。
 */
