...
/*
 * Bonsai 系 dense デコーダ GGUF — AMD GPU ROCm / HIP + rocWMMA 版（hipBLAS 不要）。
 *
 * 対象: Bonsai-8B-Q1_0.gguf（Q1_0 g128 + F32 norm 等）。テキスト処理のみ。
 * GPU 方針は gpu-cuda / gpu-rocm と同趣旨（Q1_0 + Q8_0 GEMV、Flash Attention）。
 * ビルド: gpu-rocm-wmma/Makefile（hipcc、GPU_ARCH は rocminfo 自動検出）。
 *
 * gpu-rocm-wmma — gpu-rocm の Prefill Attention QK^T を rocWMMA 化した派生版。
 *
 * 差分概要:
 *   - Prefill: flash_attn_prefill_wmma_gqa_kernel（QK^T=WMMA, PV=スカラー）
 *   - Decode:  gpu-rocm と同一（flash_attn_gqa_kernel）
 *   - 線形層:  gpu-rocm と同一（Q1_0 GEMV、WMMA 非対応）
 *
 * 正確性: WMMA QK + スカラー PV で gpu-rocm と同等の生成を確認済み。
 * 性能:   短プロンプトでは Prefill 並列度低下により gpu-rocm より遅い場合あり。
 *         詳細は kernels.hip 先頭の開発知見コメント参照。
 */
...
static void write_benchmark_log(const BenchLogInfo *info) {
    ...
    fprintf(f, "# bonsai-gpu-rocm benchmark log\n");
...
int main(int argc, char *argv[]) {
    ...
    /*
     * AMD GPU (ROCm/HIP)。重みは起動時に VRAM へアップロード。
     */
