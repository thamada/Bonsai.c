...
└── bonsai-8b/
    ...
    │   ├── Makefile
    │   └── main.c
    ├── gpu-cuda/          # 付録（README 末尾・Q1_0×Q8_0）
    ├── gpu-cuda-nvfp4/    # 付録（README 末尾・NVFP4 + CUTLASS）
    ├── gpu-rocm/          # 付録（README 末尾）
    └── gpu-rocm-wmma/     # 付録（README 末尾・Prefill WMMA 実験）
```

推論コードは **`bonsai-8b/cpu/`**（参照・単スレッド）が基準です。並列版は **`bonsai-8b/cpu-omp/`**、CPU 最適化版は **`bonsai-8b/cpu-blas/`** です。GPU 向けは **`gpu-cuda`**（NVIDIA・Q1_0）、**`gpu-cuda-nvfp4`**（NVIDIA・NVFP4 Tensor Core）、**`gpu-rocm`**（AMD）、**`gpu-rocm-wmma`**（AMD・Prefill Attention WMMA 実験）が付録としてあります（本文末尾）。
...
- 学習・ファインチューニング  
- **AMD NPU（XDNA2 等）** や **Vulkan / Metal** など（**`gpu-cuda`** / **`gpu-cuda-nvfp4`** / **`gpu-rocm`** / **`gpu-rocm-wmma`** は付録の参考実装。本文の主眼は CPU 3 バリアント）  
...
困ったときは、`bonsai-8b/Makefile`（**`model` のみ**）と各サブディレクトリの Makefile（`cpu/`、`cpu-blas/`、`gpu-cuda/`、`gpu-cuda-nvfp4/` 等）、および実行時のモデルパスを確認してください。

---

## NVIDIA CUDA 実装（`gpu-cuda`）について

**`bonsai-8b/gpu-cuda/` は本リポジトリの目的（単一 C ソース・依存最小）から外れた付録**です。`main.c` + `kernels.cu` + `gpu.h` に分かれ、線形層は **Q1_0×Q8_0 GEMV** のみ（**NVFP4 経路なし**）。**CUDA Toolkit・NVIDIA ドライバ・GPU 実機**が必要です。**`make log` / `make log.push`** でベンチマーク履歴を **`gpu-cuda/Makefile` に記録**できます。推論終了時、**`BENCH_LOG_FILE`**（既定 **`/tmp/benchmark.log`**）には tok/s に加え **VRAM 内訳**（**`GpuVramProfile`** / **`gpu_model_vram_profile`**）も key=value で出力されます。Blackwell 向け **NVFP4 Tensor Core** は別付録 **`gpu-cuda-nvfp4/`** を参照してください。参照すべき最小実装は **`cpu/main.c`** です。
...
├── Makefile
├── main.c
├── kernels.cu          # gpu_model_vram_profile 含む
...
`cpu-blas` と同じ方針です。重みは **Q1_0 のまま VRAM 常駐**（事前 dequant しない）。**`make run`**（Blackwell **`sm_120a`**）および **`make build`**（**`nvidia-smi` による GPU 自動検出**）はいずれも本経路です。**RTX 5090 等では `sm_120` / `sm_120a` ネイティブ必須**（PTX **`compute_86` JIT は推論が壊れます**）。**NVFP4** は下記 **`gpu-cuda-nvfp4`** 節を参照。
...
|---|---|---|
| GPU 自動検出でビルド | `make build` | **`nvidia-smi`** で **`CUDA_GENCODE`**（RTX 50 → **`sm_120`** 等） |
| Blackwell + Q1_0（既定） | `make` / `make run` | CUDA 13 + **`sm_120a`**（初回 **sudo**；2 回目以降 **apt スキップ**） |
| ベンチ履歴の表示 | `make log` | |
| ベンチ実行＋履歴追記 | `make log.push` | 例: `make log.push BENCH_N=64` |

```bash
cd bonsai-8b/gpu-cuda
make run              # 既定: blackwell（sm_120a, Q1_0）→ 推論（CUDA 13 済みなら apt スキップ）
# 長プロンプトベンチ: make log.push → make log
```

成功すると **`bonsai-gpu-cuda`** がこのディレクトリにできます。

**`CUDA_GENCODE` / `FA_BR`** を変更したあと古い **`kernels.o`** が残らないよう、Makefile は **`.build_config.stamp`** で再コンパイルを制御します。**`make clean`** でスタンプも削除されます。

**`make build`** は **`nvidia-smi`** で先頭 GPU の compute capability から **`CUDA_GENCODE`** / **`FA_BR`** を自動選択します（ビルド時に `GPU_CCAP` / `CUDA_GENCODE` を表示）。上書き例:

```bash
make build CUDA_GENCODE=arch=compute_90,code=sm_90
# RTX 5090 では compute_86 PTX を使わない（推論が文字化け）
```
...
```bash
cd bonsai-8b/gpu-cuda
make run PROMPT="Hello"          # Blackwell sm_120a + Q1_0（既定）
```
...
**`gpu-rocm`** / **`gpu-cuda-nvfp4`** と同趣旨。長プロンプト（ChatML 後 **約 130 トークン**）+ decode **128** トークンのベンチを実行し、結果を **`gpu-cuda/Makefile`** に追記できます。第 2 列は **`GPU_LABEL`**（**`nvidia-smi` の compute capability**、例: **`sm_120`**）。
...
**注意:** **`make log.push` は `gpu-cuda/Makefile` を書き換えます**。コミット前に `git diff` で差分を確認してください。表の **`total_tps`** は **推論区間のみ**（重みの VRAM アップロードは含みません）。

**`BENCH_LOG_FILE` の VRAM 項目**（推論終了時）: **`vram_total`**、**`vram_device_used`** / **`vram_device_total`**（**`cudaMemGetInfo`**、各 **bytes** と **`_mib`**）、**`[vram_breakdown]`** セクション（Q1_0 embedding / F32 norm / Q1_0 線形重み / KV / decode 活性 / prefill バッチ）。Makefile の **`BENCH_LOG`** 履歴には tok/s のみ追記され、VRAM 内訳は **`BENCH_LOG_FILE`** を参照してください。
...
#### Q1_0 GPU 短プロンプト（Blackwell ネイティブ、`make run`）

**CUDA 13**・**`sm_120a`**・**`FA_BR=32`**。`-p "Hello, how are you?"`（2026-05-28 実測）。

| バイナリ | prefill tok/s | decode tok/s | 備考 |
|---|---:|---:|---|
| `gpu-cuda/bonsai-gpu-cuda` | **~312** | **~47** | **`make run`**（**`sm_120a`**） |

#### Q1_0 GPU 長プロンプト（`make log.push`）

| 項目 | 値 |
|---|---|
| GPU | **sm_120**（RTX 5090。**`make log` の GPU 列**） |
| ワークロード | 長文プロンプト（ChatML 後 **130** トークン）+ decode **128** トークン（**`make log.push`** 既定: **`-n 128 -t 0 -s 42`**） |
| 表の指標 | **`/tmp/benchmark.log`**（または **`BENCH_LOG_FILE`**）の **`prefill_tps` / `decode_tps` / `total_tps`** — 推論区間のみ |
| 再現 | `bonsai-8b/gpu-cuda/` で **`make log.push`** → **`make log`** |

| 計測日時 | GPU | prefill tok/s | decode tok/s | total tok/s | 備考 |
|---|---|---:|---:|---:|---|
| 2026-05-28 17:35 | **sm_120** | **412.61** | **43.12** | **78.58** | 130+128 トークン（**`make log.push`**） |
| 2026-05-28 17:43 | **sm_120** | **411.64** | **43.16** | **78.62** | 同上（2 回目） |
| 2026-05-28 20:51 | **sm_120** | **413.24** | **43.17** | **78.66** | 同上（3 回目） |

**VRAM 内訳**（上記 **2026-05-28 20:51** 計測・3 回目。**`BENCH_LOG_FILE`** の **`[vram_breakdown]`**。`-l` 既定 **`max_seq`** 条件）:

| 項目 | bytes | MiB | 備考 |
|---|---:|---:|---|
| **`vram_total`**（理論合計） | 1,408,171,860 | **1342.94** | 下記カテゴリの合計 |
| **`vram_device_used`** | 2,226,388,992 | **2123.25** | **`cudaMemGetInfo`**（CUDA ランタイム等を含む場合あり） |
...
| `vram_weights_q1_linear` | 1,064,109,888 | **1014.81** | Q1_0 線形重み（**`wq`〜`down`** + LM head） |
| `vram_kv_cache` | 150,994,944 | **144.00** | **`kc` / `vc`**（`-l` 依存） |
| `vram_decode_activations` | 792,788 | **0.76** | 単トークン decode 用バッファ |
| `vram_prefill_batch` | 103,680,000 | **98.88** | prefill バッチ（**`batch_cap = max_seq`**） |

線形重み（**~1015 MiB**）が理論 VRAM の大半。KV（**~144 MiB**）は **`-l`** を増やすと比例して増えます。**`vram_device_used`** は **`vram_total`** より大きいことがあります（ドライバ／CUDA 割当の差）。

**PTX `compute_86` JIT** は RTX 5090 では**推論が文字化け**するため非推奨。

上記 **短プロンプト表**・**NVFP4 長プロンプト表**とは**直接比較しない**でください（プロンプト長・トークン数・経路が異なります）。

同一プロンプト・`-t 0` で、**Q1_0 構成は `cpu-blas` と同じ生成テキスト**（`Hello! I'm Bonsai, an AI assistant developed by PrismML.`）。**NVFP4** の数値は **`gpu-cuda-nvfp4`** 節を参照。

### よくあるトラブル（CUDA 版）

**CUDA / `nvcc` が見つからない:** CUDA Toolkit（**`nvcc`**）と NVIDIA ドライバをインストールし、`gpu-cuda` で `make build` または `make run`（Blackwell）を再実行してください。**RTX 50 系**は **CUDA 13** と **`sm_120` / `sm_120a` ネイティブ**が必要です（**PTX `compute_86` は使わない**）。**`make build`** は **`nvidia-smi` で `CUDA_GENCODE` を自動選択**します。

**推論が文字化け / `flash_attn … unsupported toolchain`:** Blackwell で **PTX JIT**（例: `CUDA_GENCODE=arch=compute_86,code=compute_86`）を使っていないか確認し、`make clean && make run` で再ビルドしてください。

**`make run` が毎回 apt する:** CUDA 13 が既に入っていれば 2 回目以降はスキップされます。強制再インストールは `make blackwell FORCE_CUDA_APT=1`。

プロンプトが `[prompt length … exceeds max_seq …]` で止まる場合は **`-l`** を大きくしてください。

**片付け:** `bonsai-8b/gpu-cuda`、`bonsai-8b/gpu-cuda-nvfp4`、`bonsai-8b/gpu-rocm`、または `bonsai-8b/gpu-rocm-wmma` で `make clean` してください。

### ソースを読む場合

6. `bonsai-8b/gpu-cuda/main.c` — ホスト側（GGUF・トークナイザ・`generate`・サンプリング・**VRAM ベンチログ**）  
7. `bonsai-8b/gpu-cuda/kernels.cu` — forward 本体・Q1_0 GEMV・Flash Attention・**`gpu_model_vram_profile`**  
8. `bonsai-8b/gpu-cuda/gpu.h` — C API（`gpu_forward_prefill` 等・**`GpuVramProfile`**）  

---

## NVIDIA CUDA NVFP4 実装（`gpu-cuda-nvfp4`）について

**`bonsai-8b/gpu-cuda-nvfp4/`** は **`gpu-cuda`** と同じ GGUF・CLI・prefill/decode 分割を持ち、**線形層だけ**を **NVFP4 Tensor Core + CUTLASS** に置き換えた別付録です。成果物は **`bonsai-gpu-cuda-nvfp4`**。**CUTLASS v4.5.1** が必要です（旧 v3.9.0 は RTX 5090 で GEMM `initialize` 失敗）。**`make log` / `make log.push`** でベンチマーク履歴を **`gpu-cuda-nvfp4/Makefile` に記録**できます。推論終了時、**`BENCH_LOG_FILE`**（既定 **`/tmp/benchmark.log`**）には tok/s に加え **VRAM 内訳**（**`GpuVramProfile`** / **`gpu_model_vram_profile`**）も key=value で出力されます。技術詳細は **`doc/design.md`** の「実行時の挙動（gpu-cuda-nvfp4）」を参照してください。
...
├── Makefile
├── main.c
├── kernels.cu          # 線形層は常に gpu_mm_fp4 → fp4_bonsai_mm
├── gpu.h               # GpuVramProfile（NVFP4 向けフィールド）
...
**NVFP4:** Blackwell Tensor Core 向け **4 bit 浮動小数**。要素は **E2M1**（4 bit）、**16 要素**ごとに **UE4M3** スケール（8 bit）。GGUF は **Q1_0** のまま、**起動時に BF16 復元 → NVFP4 キャッシュ**して GEMM に渡します。

| ファイル | 役割 |
|---|---|
| `fp4_gemm.cu` | CUTLASS **SM120 block-scaled NVFP4 GEMM**。起動時 **`fp4_gemm_prealloc`**。各 GEMM 前 **`fp4_gemm_init(M,N,K)`**。**`fp4_weight_cache_vram_bytes`** / **`fp4_gemm_vram_bytes`** |
| `fp4_bonsai.cu` | Q1_0 → NVFP4 重みキャッシュ、F32 ↔ BF16 ↔ GEMM。活性化パッドは **`M_act`/`M_pad` 分離**。**`fp4_bonsai_vram_bytes`** |
| `kernels.cu` | 常に **`fp4_bonsai_mm`**（`#ifdef` なし）。**`gpu_model_vram_profile`** |

**対象レイヤ:** `wq`〜`down` と `output`（LM head）。**`M`/`N`/`K` は 128 倍数**。起動時 stderr に `GPU: FP4 Tensor Core path enabled` が出れば有効。

E2M1 の 8 段階・ビット図の全文は旧 **`gpu-cuda` 付録**と同内容です（`doc/design.md` にも記載）。

### ビルド・実行

| 目的 | コマンド（`bonsai-8b/gpu-cuda-nvfp4/`） |
|---|---|
| CUTLASS 取得 | `make cutlass` |
| Blackwell + NVFP4（既定） | `make` / `make run` |
| ベンチ履歴の表示 | `make log` |
| ベンチ実行＋履歴追記 | `make log.push`（例: `make log.push BENCH_N=64`） |

```bash
cd bonsai-8b/gpu-cuda-nvfp4
make cutlass            # 初回または CUTLASS 更新時
make run                # blackwell → sm_120a + NVFP4
./bonsai-gpu-cuda-nvfp4 ../Bonsai-8B-Q1_0.gguf -p "Hello"
```

CUTLASS を更新した場合: `rm -rf third_party/cutlass && make cutlass`。

### ベンチマーク記録（`make log` / `make log.push`）

**`gpu-rocm`** と同趣旨。長プロンプト（ChatML 後 **約 130 トークン**）+ decode **128** トークンのベンチを実行し、結果を **`gpu-cuda-nvfp4/Makefile`** に追記できます。第 2 列は **`GPU_LABEL`**（**`nvidia-smi` の compute capability**、例: **`sm_120`**）。
...
**注意:** **`make log.push` は `gpu-cuda-nvfp4/Makefile` を書き換えます**。コミット前に `git diff` で差分を確認してください。表の **`total_tps`** は **推論区間のみ**（重みの VRAM アップロードは含みません）。

**`BENCH_LOG_FILE` の VRAM 項目**（推論終了時）: **`vram_total`**、**`vram_device_used`** / **`vram_device_total`**（**`cudaMemGetInfo`**、各 **bytes** と **`_mib`**）、**`[vram_breakdown]`** セクション（Q1_0 embedding / F32 norm / NVFP4 線形重み / KV / decode 活性 / prefill バッチ / FP4 GEMM スクラッチ）。Makefile の **`BENCH_LOG`** 履歴には tok/s のみ追記され、VRAM 内訳は **`BENCH_LOG_FILE`** を参照してください。

### 参考ベンチマーク（NVFP4、RTX 5090）
...
| ワークロード | 長文プロンプト（ChatML 後 **130** トークン）+ decode **128** トークン（**`make log.push`** 既定: **`-n 128 -t 0 -s 42`**） |
| 表の指標 | **`/tmp/benchmark.log`**（または **`BENCH_LOG_FILE`**）の **`prefill_tps` / `decode_tps` / `total_tps`** — 推論区間のみ |
| 再現 | `bonsai-8b/gpu-cuda-nvfp4/` で **`make log.push`** → **`make log`** |

| 計測日時 | GPU | prefill tok/s | decode tok/s | total tok/s | 備考 |
|---|---|---:|---:|---:|---|
| 2026-05-28 18:48 | **sm_120** | **5751.92** | **64.12** | **127.80** | 130+128 トークン（**`make log.push`**） |
| 2026-05-28 20:30 | **sm_120** | **5762.22** | **64.14** | **127.84** | 同上（2 回目） |
| 2026-05-28 20:30 | **sm_120** | **5757.93** | **64.20** | **127.96** | 同上（3 回目） |

**VRAM 内訳**（上記 **2026-05-28 20:30** 計測・3 回目。**`BENCH_LOG_FILE`** の **`[vram_breakdown]`**。`-l` 既定 **`max_seq`** 条件）:

| 項目 | bytes | MiB | 備考 |
|---|---:|---:|---|
| **`vram_total`**（理論合計） | 5,975,701,524 | **5698.87** | 下記カテゴリの合計 |
| **`vram_device_used`** | 6,752,043,008 | **6439.25** | **`cudaMemGetInfo`**（CUDA ランタイム等を含む場合あり） |
| **`vram_device_total`** | 33,669,513,216 | **32109.75** | GPU 全体 VRAM |
| `vram_weights_q1_embd` | 87,361,344 | **83.31** | Q1_0 **`token_embd`** |
| `vram_weights_f32_norm` | 1,232,896 | **1.18** | F32 norm 重み |
| `vram_weights_fp4` | 4,256,464,896 | **4059.28** | NVFP4 線形重みキャッシュ（**`wq`〜`down`** + LM head） |
| `vram_kv_cache` | 150,994,944 | **144.00** | **`kc` / `vc`**（`-l` 依存） |
| `vram_decode_activations` | 792,788 | **0.76** | 単トークン decode 用バッファ |
| `vram_prefill_batch` | 103,680,000 | **98.88** | prefill バッチ（**`batch_cap = max_seq`**） |
| `vram_fp4_gemm_scratch` | 1,375,174,656 | **1311.47** | BF16 活性／出力 + CUTLASS workspace 等 |

NVFP4 線形キャッシュ（**~4059 MiB**）と GEMM スクラッチ（**~1311 MiB**）が理論 VRAM の大半。**Q1_0 経路**（上記 **`gpu-cuda`** 長プロンプト表・理論 **~1343 MiB**）より大きいが、Tensor Core GEMM 用の作業領域を含む。KV・embedding・prefill バッチは Q1_0 版と同程度。

上記 **Q1_0 GPU（`make run`・短プロンプト）** 表とは**直接比較しない**でください（プロンプト長・トークン数が異なります）。

### よくあるトラブル（NVFP4）

**`fp4_gemm_run_cached: initialize: Error Internal`:** **CUTLASS v4.5.1** を再取得してください（`rm -rf third_party/cutlass && make cutlass`）。CUDA 13 + **`sm_120a`** ネイティブであることも確認してください。

---

## AMD ROCm / HIP 実装（`gpu-rocm`）について

**`bonsai-8b/gpu-rocm/` も本リポジトリの目的（単一 C ソース・依存最小）から外れた付録**です。`main.c` + `kernels.hip` + `gpu.h`（**`gpu-cuda/gpu.h` をベースに `gpu_get_device_desc` を追加した C API**）に分かれ、**ROCm（`hipcc`）・AMD GPU ドライバ・実機**が必要です。**hipBLAS 不要**（線形層は Q1_0×Q8_0 のカスタム HIP カーネル）。**NVFP4 経路はありません**（アルゴリズムは **`gpu-cuda` の Q1_0 経路**と同系）。**`make log` / `make log.push`** でベンチマーク履歴を **`gpu-rocm/Makefile` に記録**できます。
...
`cpu-blas` / **`gpu-cuda`（Q1_0 経路）** と同じ GGUF・CLI・**prefill バッチ + decode 逐次**（**`gpu_forward_prefill`** / **`gpu_forward`**）。ホストは GGUF mmap・トークナイザ・サンプリング、デバイスは **Flash Attention**（K/V shared staging、**`FA_BR`** タイル）と **Q8_0 活性化 + Q1_0 GEMV**。VRAM 配置の考え方は上記 CUDA 付録の表と同趣旨（API は **HIP** / **`hipMalloc`**）。
...
| 項目 | 値 |
|---|---|
| GPU | **gfx1201** / **gfx1100** 等（ROCm。**`GPU_ARCH`**。表の **GPU_ARCH** 列） |
| OS | Linux |
...
**CPU 表**（`-p "Hello" -n 16`）や **CUDA 付録**（prefill 18 + decode 16）とは**プロンプト長・トークン数が異なる**ため、数値をそのまま横並び比較しないでください。**GPU_ARCH** 列（**gfx1201** vs **gfx1100**）は計測 GPU・ホストが異なるため、表内でも**条件を揃えてから**比較してください。短いプロンプトでは手動で `./bonsai-gpu-rocm ... -p "Hello" -n 16 -t 0` を実行し、stderr の **`--- throughput ---`** を参照してください。
