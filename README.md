...
└── qwen3-8b/
    ...
    ├── main-omp.c
    ├── main-rocm.c
    ├── main-rocm-fullgpu-flash-opt2.c
...
`main-rocm-fullgpu-flash-opt2.c` をビルドするターゲットもあります。

```bash
make build.rocm.fullgpu.flash.opt2 GPU_ARCH=gfx1201
./qwen3-rocm-fullgpu-flash-opt2 Qwen_Qwen3-VL-8B-Instruct-IQ2_M.gguf -p "Hello" -n 32
```

現在の設計では `main-rocm.c` と同じロジックを持つ別名ソースとして扱います。
