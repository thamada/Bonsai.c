...
| 種別 | 内容 | 形式 |
|---|---|---|
| 重み | `token_embd`, 各層 `wq/wk/wv/wo/gate/up/down`, `output` | Q1_0（g128）。**`BONSAI_FP4=1`** ビルド時のみ線形層を起動時に **NVFP4 + block scale の GPU キャッシュ**へ変換（下記） |
...
#### Q1_0 GEMV（`gpu_mm` / `gpu_mm_batch`）— 既定

`cpu-blas` と同じ方針です。重みは **Q1_0 のまま VRAM 常駐**（事前 dequant しない）。**`make run`**（Blackwell **`sm_120a`**、**`BONSAI_FP4=0`**）と **`make run.no-fp4`**（**`BONSAI_FP4=0`**、**`make build` の GPU 自動検出**）のいずれも本経路です。**RTX 5090 等では `sm_120` ネイティブ必須**（PTX **`compute_86` JIT は推論が壊れます**）。**`BONSAI_FP4=1`** でビルドしたときだけ次節の NVFP4 に切り替わります。
...
#### NVFP4 + CUTLASS（Blackwell、手動 `BONSAI_FP4=1`）
...
**`BONSAI_FP4=1`** でビルドしたときのみ有効（**`make run` / `make blackwell` 既定は `BONSAI_FP4=0`** — NVFP4 を使う場合は「ビルド」節の手動例を参照）。**`main.c` は NVFP4 を直接呼びません** — GGUF 読み込み・`generate`・サンプリングは従来どおりで、**`kernels.cu` 内の線形層 GEMV/GEMM だけ**が FP4 Tensor Core 処理に切り替わります。Embedding・RMSNorm・RoPE・Flash Attention・SwiGLU・KV キャッシュは変更ありません。

| ファイル | 役割 |
|---|---|
| `fp4_gemm.cu` | [CUTLASS](https://github.com/NVIDIA/cutlass) の **SM120 block-scaled NVFP4 GEMM**（Example 79a ベース）。起動時 **`fp4_gemm_prealloc(max_M,max_N,max_K)`** でバッファ確保。**`fp4_gemm_run_cached`** は prealloc 済み・形状上限内のみ実行。 |
...
**`make run` 既定・`make run.no-fp4`** では本節は無効で、上記「Q1_0 GEMV」（Q8_0 活性化 + `mm_q1_0_*` カーネル）のみです。
...
|---|---|---|
| Q1_0・FP4 無効（GPU 自動検出） | `make build` / `make run.no-fp4` | **`nvidia-smi`** で **`CUDA_GENCODE`**（RTX 50 → **`sm_120`** 等） |
| Blackwell + Q1_0（既定） | `make` / `make run` | CUDA 13 + **`sm_120a`** + **`BONSAI_FP4=0`**（初回 **sudo**；2 回目以降 **apt スキップ**） |
| Blackwell + NVFP4 | 下記手動ビルド | **`make cutlass`** + **`BONSAI_FP4=1`** |

```bash
cd bonsai-8b/gpu-cuda
make run              # 既定: blackwell（sm_120a, Q1_0）→ 推論（CUDA 13 済みなら apt スキップ）
# make run.no-fp4     # build（GPU 自動検出, FP4 無効）→ 推論
```

Blackwell で NVFP4 を使う場合（任意）:
...
**`CUDA_GENCODE` / `BONSAI_FP4` / `FA_BR`** を変更したあと古い **`kernels.o`** が残らないよう、Makefile は **`.build_config.stamp`**（内容 `CUDA_GENCODE|fp4=…|fa=…`）で **`kernels.o` / `fp4_*.o`** の再コンパイルを制御します。**`make clean`** でスタンプも削除されます。

**`make build`** / **`run.no-fp4`** は **`nvidia-smi`** で先頭 GPU の compute capability から **`CUDA_GENCODE`** / **`FA_BR`** を自動選択します（ビルド時に `GPU_CCAP` / `CUDA_GENCODE` を表示）。上書き例:
...
# make run.no-fp4 PROMPT="Hello" # GPU 自動検出 + Q1_0（FP4 無効）
```
...
| コマンド | **`make run`**（**`sm_120a`**・Q1_0）または **`make run.no-fp4`**（GPU 自動検出・Q1_0）。FP4 表は **`BONSAI_FP4=1`** 手動ビルド |
...
#### FP4 Tensor Core 有効（手動 `BONSAI_FP4=1`）

Blackwell（RTX 50 系）向け。**CUDA 13**・**`sm_120a`** ネイティブ・**`BONSAI_FP4=1`**（NVFP4 + CUTLASS）・**`FA_BR=32`**。**`make run` 既定は FP4 無効**（下記 Q1_0 表参照）。
...
#### Q1_0 GPU（Blackwell ネイティブ、`make run` / `make run.no-fp4`）

**CUDA 13**・**`sm_120` / `sm_120a`**・**`BONSAI_FP4=0`**・**`FA_BR=32`**。`-p "Hello, how are you?"`（2026-05-28 実測）。

| バイナリ | prefill tok/s | decode tok/s | 備考 |
|---|---:|---:|---|
| `gpu-cuda/bonsai-gpu-cuda` | **~312** | **~47** | **`make run`**（**`sm_120a`**） |

**PTX `compute_86` JIT** は RTX 5090 では**推論が文字化け**するため非推奨（2026-05-21 の PTX 表は Ampere/Ada 向け参考）。

同一プロンプト・`-t 0` で、**Q1_0 構成は `cpu-blas` と同じ生成テキスト**（`Hello! I'm Bonsai, an AI assistant developed by PrismML.`）。2026-05-21 計測では **手動 `BONSAI_FP4=1`** 時に decode **約 1.9 倍**、prefill **約 4.7 倍**（いずれも tok/s）程度でした。
...
**推論が文字化け / `flash_attn … unsupported toolchain`:** Blackwell で **PTX JIT**（例: `CUDA_GENCODE=arch=compute_86,code=compute_86`）を使っていないか確認し、`make clean && make run` または `make run.no-fp4` で再ビルドしてください。
