...
**`bonsai-8b/gpu-cuda/` は本リポジトリの目的（単一 C ソース・依存最小）から外れた付録**です。`main.c` + `kernels.cu` + `gpu.h` に分かれ、線形層は **Q1_0×Q8_0 GEMV** のみ（**NVFP4 経路なし**）。**CUDA Toolkit・NVIDIA ドライバ・GPU 実機**が必要です。Blackwell 向け **NVFP4 Tensor Core** は別付録 **`gpu-cuda-nvfp4/`** を参照してください。参照すべき最小実装は **`cpu/main.c`** です。
...
#### Q1_0 GPU（Blackwell ネイティブ、`make run`）
...
6. `bonsai-8b/gpu-cuda/main.c` — ホスト側（GGUF・トークナイザ・`generate`・サンプリング）  
7. `bonsai-8b/gpu-cuda/kernels.cu` — forward 本体・Q1_0 GEMV・Flash Attention  
...
├── gpu.h               # GpuVramProfile / gpu_model_vram_profile 追加
