...
**`bonsai-8b/gpu-cuda-nvfp4/`** は **`gpu-cuda`** と同じ GGUF・CLI・prefill/decode 分割を持ち、**線形層だけ**を **NVFP4 Tensor Core + CUTLASS** に置き換えた別付録です。成果物は **`bonsai-gpu-cuda-nvfp4`**。**CUTLASS v4.5.1** が必要です（旧 v3.9.0 は RTX 5090 で GEMM `initialize` 失敗）。技術詳細は **`doc/design.md`** の「実行時の挙動（gpu-cuda-nvfp4）」を参照してください。
...
| バイナリ | prefill tok/s | decode tok/s | 備考 |
|---|---:|---:|---|
| `gpu-cuda-nvfp4/bonsai-gpu-cuda-nvfp4` | **~1365** | **~90.4** | 短プロンプト・`-n 16` 相当（2026-05-21） |
| `gpu-cuda-nvfp4/bonsai-gpu-cuda-nvfp4` | **~1767** | **~65** | **`make run`**、`-p "Hello, how are you?"`（2026-05-28） |

2026-05-21 計測では NVFP4 が当時の Q1_0+FP4 ビルドより decode **約 1.9 倍**、prefill **約 4.7 倍**（tok/s）程度でした。
