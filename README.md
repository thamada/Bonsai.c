...
`cpu-blas` と同じ方針です。重みは **Q1_0 のまま VRAM 常駐**（事前 dequant しない）。**`make run`**（Blackwell **`sm_120a`**、**`BONSAI_FP4=0`**）と **`make run.no-fp4`**（PTX **`compute_86`** JIT）のいずれも本経路です。**`BONSAI_FP4=1`** でビルドしたときだけ次節の NVFP4 に切り替わります。
...
|---|---|---|
| 汎用 GPU（PTX JIT、Q1_0） | `make build` / `make run.no-fp4` | PTX `compute_86` + ドライバ JIT |
| Blackwell + Q1_0（既定） | `make` / `make run` | CUDA 13 + **`sm_120a`** + **`BONSAI_FP4=0`**（初回 **sudo** のことがある） |
| Blackwell + NVFP4 | 下記手動ビルド | **`make cutlass`** + **`BONSAI_FP4=1`** |

```bash
cd bonsai-8b/gpu-cuda
make run              # 既定: blackwell（sm_120a, Q1_0）→ 推論
...
| コマンド | **`make run`**（Blackwell **`sm_120a`**・Q1_0）または **`make run.no-fp4`**（PTX **`compute_86`**・Q1_0）。FP4 表は **`BONSAI_FP4=1`** 手動ビルド |
...
PTX **`compute_86`** + ドライバ JIT、Q1_0 GEMV。`gpu-cuda/` で `make run.no-fp4`（`make build` のみでリンク）。

| バイナリ | prefill tok/s | decode 時間 | decode スループット | 備考 |
|---|---:|---:|---:|---|
| `gpu-cuda/bonsai-gpu-cuda` | **~293** | 0.34 s | **~47.0 tok/s** | バッチ prefill・`-use_fast_math`（2026-05-21 実測） |

#### Q1_0 GPU（Blackwell ネイティブ、`make run` 既定）

**CUDA 13**・**`sm_120a`**・**`BONSAI_FP4=0`**・**`FA_BR=32`**。上記 PTX 表と**直接比較しない**（アーキテクチャ・JIT 有無が異なる）。数値未収録。
