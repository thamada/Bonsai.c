...
| ファイル | 役割 |
|---|---|
| `fp4_gemm.cu` | CUTLASS **SM120 block-scaled NVFP4 GEMM**。起動時 **`fp4_gemm_prealloc`**。各 GEMM 前 **`fp4_gemm_init(M,N,K)`** |
| `fp4_bonsai.cu` | Q1_0 → NVFP4 重みキャッシュ、F32 ↔ BF16 ↔ GEMM。活性化パッドは **`M_act`/`M_pad` 分離** |
| `kernels.cu` | 常に **`fp4_bonsai_mm`**（`#ifdef` なし） |
