...
| 計測 | プロンプト 20 トークン + **生成 15 トークン**（`-n 16` で EOS により 15 で打ち切り） |

| バイナリ | 合計時間 | 生成スループット | 備考 |
|---|---:|---:|---|
| `cpu-omp/bonsai-cpu-omp` | 161.7 s | **0.09 tok/s** | `-O3 -fopenmp`（`cpu-omp/Makefile` 既定） |
| `cpu-blas/bonsai-cpu-blas` | 8.4 s | **1.79 tok/s** | `-O3 -fopenmp -march=native -ffast-math`、OpenBLAS 1 スレッド |

この条件下では **`cpu-blas` が約 20 倍**のスループットでした。生成テキスト（`Hello! I'm an AI assistant. How can I help you today?`）は両バイナリで一致しています。`-ffast-math` により浮動小数の結合順は `cpu-omp` と異なり得ますが、上記試行では同一出力でした。
