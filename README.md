...
**`bonsai-8b/gpu-cuda/` は本リポジトリの目的（単一 C ソース・依存最小）から外れた付録**です。`main.c` + `kernels.cu` + `polarquant.c` + `gpu.h` に分かれ、**CUDA Toolkit・NVIDIA ドライバ・GPU 実機**が必要です。参照すべき最小実装は **`cpu/main.c`** です。
...
| KV キャッシュ（`--polarquant`） | `kc_pack`, `vc_pack`, `kc_win`, `vc_win`, `kc_win_pos`, `vc_win_pos` | 全系列 **pack**（K/V 各 **36 B/head**＝indices 32 + **per-vector scale** 4、**Pi** 回転 + Lloyd-Max **2-bit**、QJL なし）+ **直近 64 トークン F32 リング**（`FA_BR`、`win_pos[slot]==pos` でタグ照合）。フル F32 `kc`/`vc` は**確保しない**（起動ログの **72 B/head** は K+V pack 合計、pack+win で約 **5×** VRAM 削減、`attn=pack+win64`） |
...
6. **KV 書き込み** — **`--polarquant`**: `kv_pq_compress_kernel` で pack へ圧縮（**`sc=max|y_rot|`** で正規化）+ `kv_write_ring_kernel` で直近ウィンドウへ F32 コピー。未指定時は F32 `kc`/`vc` へ D2D。
7. **Attention** — `flash_attn_gqa_kernel`。**`--polarquant`**: 直近 64 位置でタグ一致時のみ `kc_win`/`vc_win` の F32 dot、それ以前は pack（K: **`q·(Pi^T·sc·centroid)`**、V: **`Pi^T`** で復元）。出力は `xb`。
...
| KV 書き込み | **`--polarquant`**: `kv_pq_compress_batch` + `kv_write_ring_batch` / 既定: `kv_write_batch`（F32） | pack + リングウィンドウ、または全系列 F32 |
| Attention | `flash_attn_prefill_gqa_kernel` | 1 block / `(token, head)`、因果マスク `npos = t + 1`（polarquant 時は pack+win） |
...
`att` 行列 `[n_heads, seq, seq]` は **materialize しません**。K/V を **64 トークン（`FA_BR`）タイル**で走査し、**online softmax** で更新します。**`--polarquant`** ではタイル内の各位置について、直近 64 トークン以内かつ `win_pos[slot]==pos` なら F32 リング、それ以外は pack 復元（**centroid×scale**）を使い分けます。
...
- **`--polarquant`**: Prefill で pack + リングを填充。Decode は `pos = n_prompt` から pack/リングに追記し Attention。**プロンプト長 ≤ 64** なら因果参照の大半が F32 ウィンドウ内。**それより長い文脈**は 2-bit pack 近似に依存（直近 64 位置は F32 で品質を確保）。
- **PolarQuant**（[arXiv:2502.02617](https://arxiv.org/abs/2502.02617)）: **Pi** 回転 + Lloyd-Max **2-bit**（QJL なし）。**`head_dim != 128`** で自動無効化。旧 **TurboQuant**（QJL 付き pack+win）は撤去済み。
- **pack のみ**（scale 未適用の実験版）ではプロンプト次第で品質崩れがあり得る。現行は **per-vector scale** + **pack+win** で F32 既定に近い応答を目指す。**`--polarquant` の decode スループットは F32 既定より低下し得る**（下表は F32 KV の計測）。
...
CLI オプション（`-p`、`-n`、`-t`、`-k`、`-s`、`-l`）は CPU 版と同様です。**`gpu-cuda` のみ** **`--polarquant`**（PolarQuant pack KV + 直近 64 トークン F32 リング、**pack+win** Attention）。
...
**`--polarquant`** 時は KV VRAM 約 **5×** 削減の一方、decode は pack 復元のため **~11–42 tok/s** 程度まで低下し得る（要再計測）。`-p "Hello, how are you?"` では F32 既定と同様の応答になることを確認済み。

同一プロンプト・`-t 0`（F32 KV）で **`cpu-blas` と同じ生成テキスト**（`Hello! I'm Bonsai, an AI assistant developed by PrismML.`）。`-ffast-math` / `-use_fast_math` により浮動小数の結合順は CPU 版と異なり得ますが、上記試行では同一出力でした。
...
7. `bonsai-8b/gpu-cuda/kernels.cu` — CUDA カーネル（`gpu_forward_prefill` / `gpu_forward`、Flash Attention、PolarQuant 圧縮・pack+win 等）  
8. `bonsai-8b/gpu-cuda/polarquant.c` — PolarQuant CPU 参照（Lloyd-Max、**Pi** 回転、テーブル生成）  
9. `bonsai-8b/gpu-cuda/gpu.h` — C API（`gpu_forward_prefill` 等、`GpuConfig.polarquant_kv`）
