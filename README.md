...
**`bonsai-8b/gpu-cuda/` は本リポジトリの目的（単一 C ソース・依存最小）から外れた付録**です。`main.c` + `kernels.cu` + `turboquant.c` + `gpu.h` に分かれ、**CUDA Toolkit・NVIDIA ドライバ・GPU 実機**が必要です。参照すべき最小実装は **`cpu/main.c`** です。
...
CLI オプション（`-p`、`-n`、`-t`、`-k`、`-s`、`-l`）は CPU 版と同様です。**`gpu-cuda` のみ** **`--turboquant`**（TurboQuant KV 圧縮バッファを追加。Attention は F32 KV のまま）。
...
7. `bonsai-8b/gpu-cuda/kernels.cu` — CUDA カーネル（`gpu_forward_prefill` / `gpu_forward`、Flash Attention、TurboQuant 圧縮等）  
8. `bonsai-8b/gpu-cuda/turboquant.c` — TurboQuant CPU 参照（PolarQuant + QJL、テーブル生成）  
9. `bonsai-8b/gpu-cuda/gpu.h` — C API（`gpu_forward_prefill` 等、`GpuConfig.turboquant_kv`）
